...
const pivotIndex = (nums) => {
  // First, calculate the total sum of array nums
  const totalSum = nums.reduce((sum, num) => sum + num, 0);
  // Keep track of left sum while iterating through input array
  let leftSum = 0;
  for (let i = 0; i < nums.length; i++) {
    // If the left sum is equal to the right sum, then return index
    if (leftSum === totalSum - leftSum - nums[i]) return i;
    // Otherwise, add to left sum and continue
    leftSum += nums[i];
  }
