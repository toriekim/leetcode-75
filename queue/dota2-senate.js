/*
  ...
  Tags: Medium, String, Greedy, Queue
...
const predictPartyVictory = (senate) => {
  // Initialize arrays to keep track of senator positions
  const radiant = [];
  const dire = [];

  // Loop through each senator & add their position respectively
  ...
  // Continue voting rounds until one party has no senators left
  while (radiant.length > 0 && dire.length > 0) {
    // If the first senator from Radiant party is before the first senator from Dire party...
    if (radiant[0] < dire[0]) {
      // Ban senator from Dire party & go to next round
      // (offset by length of senate to simulate round-robin voting)
      radiant.push(radiant[0] + senate.length);
    } else {
      // Otherwise, ban senator from Radiant party & go to next round
      dire.push(dire[0] + senate.length);
    }
    // Remove the first senator from each party array after they've voted in the current round
    radiant.shift();
    dire.shift();
  }
  // Return the winner based on which party has senators left
